...
namespace NHibernate.Test.Performance
{
	...
	public class PerformanceTest : TestCase
	{
		...
		private DriverBase driver = null;
		...
		public void Many()
		{
			...
			Console.Out.Write("NHibernate: " + hiber + "ms / Direct ADO.NET: " + adonet + "ms = Ratio: " +
			                  (((float) hiber / adonet)).ToString());
		...
		public void Simultaneous()
		{
			...
			for (int n = 2; n < 4000; n *= 2)
			{
				...
				adonet += DateTime.Now.Ticks - time;
				cp.CloseConnection(c);
				Console.Out.WriteLine("Objects " + n + " NHibernate: " + hiber + "ms / Direct ADO.NET: " + adonet + "ms = Ratio: " +
				                      (((float) hiber / adonet)).ToString());
			}
		...
		public void NHibernateOnly()
		{
			for (int n = 2; n < 4000; n *= 2)
			{
				...
				Console.WriteLine("Objects: " + n + " - NHibernate: " + hiber);
			}
		...
		public void AdoNetOnly()
		{
			...
			for (int n = 2; n < 4000; n *= 2)
			{
				...
				Console.Out.WriteLine("Objects: " + n + " Direct ADO.NET: " + adonet);
			}
		...
		private void Hibernate(ISession s, Simple[] simples, object[] ids, int N, string runname)
		{
			...
			for (int i = 0; i < N; i++)
			{
				simples[i].Name = "NH - " + i + N + runname + " - " + DateTime.Now.Ticks;
			}
...
//			}
		...
		private void DirectAdoNet(IDbConnection c, Simple[] simples, object[] ids, int N, string runname)
		{
			...
			for (int i = 0; i < N; i++)
			{
				((IDbDataParameter) update.Parameters[0]).Value = "DR - " + i + N + runname + " - " + DateTime.Now.Ticks;
