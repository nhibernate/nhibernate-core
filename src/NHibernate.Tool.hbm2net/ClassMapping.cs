...
namespace NHibernate.Tool.hbm2net
{
	public class ClassMapping:MappingElement
	{
		...
		{
			for (IEnumerator arrays = classElement.SelectNodes("urn:" + type, CodeGenerator.nsmgr).GetEnumerator(); arrays.MoveNext(); )
			{
		...
		public virtual void addImport( ClassName className )
		{
			...
			if (!className.inJavaLang() && !className.inSamePackage(generatedName) && !className.Primitive && className.PackageName != null && className.PackageName.Length > 0)
			{
				/*
				if (className.Array)
				{
