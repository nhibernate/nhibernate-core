...
namespace NHibernate.Test.CacheTest
{
	...
	public class BatchableCacheFixture : TestCase
	{
		...
		public void MultipleGetReadOnlyCollectionTest()
		{
			...
			{
				AssertMultipleGetCollectionCalls(ids, tuple.Item1, tuple.Item2, tuple.Item3);
			}
		}
		...
		public void MultipleGetReadOnlyTest()
		{
			...
			{
				AssertMultipleGetCalls<ReadOnly>(ids, tuple.Item1, tuple.Item2, tuple.Item3);
			}
		}
		...
		public void MultipleGetReadOnlyItemTest()
		{
			...
			foreach (var tuple in parentTestCases)
			{
				AssertMultipleGetCalls<ReadOnlyItem>(ids, tuple.Item1, tuple.Item2, tuple.Item3);
			}
		}
		...
		private void AssertMultipleGetCalls<TEntity>(List<int> ids, int idIndex, int[][] fetchedIdIndexes, Func<int, bool> cacheBeforeLoadFn = null)
		...
		private void AssertMultipleGetCollectionCalls(List<int> ids, int idIndex, int[][] fetchedIdIndexes, Func<int, bool> cacheBeforeLoadFn = null)
		{
			var persister = Sfi.GetCollectionPersister($"{typeof(ReadOnly).FullName}.Items");
			var cache = (BatchableCache) persister.Cache.Cache;
			cache.Clear();
