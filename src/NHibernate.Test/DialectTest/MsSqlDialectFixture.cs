...
namespace NHibernate.Test.DialectTest 
{
	...
	public class MsSqlDialectFixture : DialectFixture
	{
		...
		public void GetLimitString()
		{
			...
			Assert.AreEqual("select top 100 id, col1, col2 from someTable".ToLower(), limitedSql.ToString().ToLower(),
				"Bad limit SQL");
			...
			limitedSql = d.GetLimitString(builder.ToSqlString(), 0, 100);
			Assert.AreEqual("select top 100 id, col1, col2 from someTable where id in (select id from othertable)".ToLower(), limitedSql.ToString().ToLower(),
				"Bad limit SQL");
			...
			limitedSql = d.GetLimitString(builder.ToSqlString(), 0, 100);
			Assert.AreEqual("select distinct top 100 id, col1, col2 from someTable".ToLower(), limitedSql.ToString().ToLower(),
				"Bad limit SQL");
