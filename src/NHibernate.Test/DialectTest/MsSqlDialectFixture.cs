...
namespace NHibernate.Test.DialectTest 
{
	...
	public class MsSqlDialectFixture : DialectFixture
	{
		...
		public void GetLimitString()
		{
			...
			Assertion.AssertEquals("Bad limit SQL", "select top 100 id, col1, col2 from someTable".ToLower(), limitedSql.ToString().ToLower());
			...
			limitedSql = d.GetLimitString(builder.ToSqlString(), 0, 100);
			Assertion.AssertEquals("Bad limit SQL", "select top 100 id, col1, col2 from someTable where id in (select id from othertable)".ToLower(), limitedSql.ToString().ToLower());
			// now the distinct case
			// first with simple string
			builder = new SqlStringBuilder();
			builder.Add("select distinct id, col1, col2 from someTable");
			limitedSql = d.GetLimitString(builder.ToSqlString(), 0, 100);
			Assertion.AssertEquals("Bad limit SQL", "select distinct top 100 id, col1, col2 from someTable".ToLower(), limitedSql.ToString().ToLower());
			// now with parts
			builder = new SqlStringBuilder();
