...
namespace NHibernate.Test.NHSpecificTest.NH2228
{
	...
	public class FixtureAsync : BugTestCase
	{
		...
		private async Task DeleteChildUsingAnotherSessionAsync(int childIdToDelete, CancellationToken cancellationToken = default(CancellationToken))
		{
			using (var client2 = Sfi.OpenStatelessSession())
			using (var tx2 = client2.BeginTransaction())
			{
				await (client2.CreateQuery("delete from Child c where c.Id = :pChildId").SetInt32("pChildId", childIdToDelete).ExecuteUpdateAsync(cancellationToken));
