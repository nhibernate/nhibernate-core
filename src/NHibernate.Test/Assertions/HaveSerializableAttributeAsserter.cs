		...
		public override string Message
		{
			get
			{
				if (clazz.IsInterface)
					FailureMessage.WriteExpectedLine(string.Format("The class {0} is an interface.", clazz.FullName));
				else
					FailureMessage.WriteExpectedLine(string.Format("The class {0} is not marked as Serializable.", clazz.FullName));
