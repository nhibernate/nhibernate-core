...
{
	/// <summary>
	/// A <see cref="LazyInitiliazer"/> for use with the Castle Dynamic Class Generator.
	/// </summary>
	[Serializable]
	public class CastleLazyInitializer : LazyInitializer, Castle.DynamicProxy.IInterceptor//, ISerializable
	{
		private static readonly log4net.ILog log = log4net.LogManager.GetLogger( typeof(CastleLazyInitializer) );
		...
		public object Intercept(IInvocation invocation, params object[] args)
		{
			...
			if( result==InvokeImplementation ) 
			{
				...
				//return invocation.Method.Invoke( GetImplementation(), args );
			}
			else 
			{
				return result;
