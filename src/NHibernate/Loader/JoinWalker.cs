...
namespace NHibernate.Loader
{
	public class JoinWalker
	{
				...
				switch( config )
				{
					...
					case FetchMode.Default:
						if( type.IsEntityType )
						{
							...
							IEntityPersister persister = factory.GetPersister( entityType.AssociatedClass );
		...
		/// </summary>
		protected virtual bool IsJoinedFetchEnabled( IAssociationType type, FetchMode config, Cascades.CascadeStyle cascadeStyle )
		{
		...
		private class AssociationKey 
		{
			...
			public override bool Equals( object other ) 
			{
				...
				return that.table.Equals( table ) && ArrayHelper.Equals( columns, that.columns );
			}
