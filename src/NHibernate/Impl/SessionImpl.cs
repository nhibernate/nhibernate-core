...
namespace NHibernate.Impl {
	...
	/// </remarks>
	[Serializable]
	public class SessionImpl : ISessionImplementor  {
		...
		[NonSerialized] private IDictionary arrayHolders; //key=array, value=ArrayHolder
		...
		public class CollectionEntry : ICollectionSnapshot {
			...
			public void PostFlush(PersistentCollection collection) {
				if (!processed) throw new AssertionFailure("Hibernate has a bug processing collections");
		...
		internal SessionImpl(IDbConnection connection, SessionFactoryImpl factory, bool autoclose, long timestamp, IInterceptor interceptor) {
			...
			entries = IdentityMap.Instantiate();
			collections = IdentityMap.Instantiate();
			arrayHolders = IdentityMap.Instantiate();
...
#warning SimpleTest runs until this point: persister is null somehow!!!!
		...
		private bool AreTablesToBeUpdated(IList tables) {
			return AreTablesToUpdated( updates.Values, tables) ||
				...
				if (status!=LOADING && status!=GONE) {
					...
					if ( persister.IsMutable &&
						...
						if (status==LOADED) {
							...
							TypeFactory.DeepCopy(values, types, persister.PropertyUpdateability, copiedValues);
						...
						if ( persister.IsVersioned ) {
							if (status!=DELETED) entry.nextVersion = Versioning.Increment( entry.lastVersion, persister.VersionType );
							Versioning.SetVersion(values, entry.CurrentVersion, persister);
						}

						// schedule the update
						updates.Add(
							new Key(entry.id, persister),
							new ScheduledUpdate( entry.id, values, dirtyProperties, entry.lastVersion, obj, persister, this )
							);
					}
					...
					if (status==DELETED) {
						...
						// search for collections by reachability, updating their roles.
		...
		private void PreFlushCollections() {
			...
			// initialize dirty flags for arrays + collectiosn with composte elements
