				...
				if (RowDeleteEnabled)
				{
					...
					foreach (object entry in entries)
					{
						if (await (collection.NeedsUpdatingAsync(entry, i, ElementType, cancellationToken)).ConfigureAwait(false))
						{
							...
							if (useBatch)
							{
								st = await (batcher.PrepareBatchCommandAsync(SqlDeleteRowString.CommandType, sql.Text,
																		 SqlDeleteRowString.ParameterTypes, cancellationToken)).ConfigureAwait(false);
							}
							else
							{
								st = await (batcher.PrepareCommandAsync(SqlDeleteRowString.CommandType, sql.Text,
																	SqlDeleteRowString.ParameterTypes, cancellationToken)).ConfigureAwait(false);
							}
							...
							try
							{
								...
								if (useBatch)
								{
									await (batcher.AddToBatchAsync(deleteExpectation, cancellationToken)).ConfigureAwait(false);
								}
								else
								{
									deleteExpectation.VerifyOutcomeNonBatched(await (batcher.ExecuteNonQueryAsync(st, cancellationToken)).ConfigureAwait(false), st);
								}
							}
							catch (Exception e)
							{
				...
				if (RowInsertEnabled)
				{
					...
					foreach (object entry in entries)
					{
						if (await (collection.NeedsUpdatingAsync(entry, i, ElementType, cancellationToken)).ConfigureAwait(false))
						{
							...
							if (useBatch)
							{
								st = await (batcher.PrepareBatchCommandAsync(SqlInsertRowString.CommandType, sql.Text,
																		 SqlInsertRowString.ParameterTypes, cancellationToken)).ConfigureAwait(false);
							}
							else
							{
								st = await (batcher.PrepareCommandAsync(SqlInsertRowString.CommandType, sql.Text,
																	SqlInsertRowString.ParameterTypes, cancellationToken)).ConfigureAwait(false);
							}
							...
							try
							{
								...
								if (useBatch)
								{
									await (batcher.AddToBatchAsync(insertExpectation, cancellationToken)).ConfigureAwait(false);
								}
								else
								{
									insertExpectation.VerifyOutcomeNonBatched(await (batcher.ExecuteNonQueryAsync(st, cancellationToken)).ConfigureAwait(false), st);
								}
							}
							catch (Exception e)
							{
