			...
			try
			{
				using (var reader = await (resultSetsCommand.GetReaderAsync(_timeout, cancellationToken)).ConfigureAwait(false))
				{
					...
					for (int i = 0; i < translators.Count; i++)
					{
						...
						await (translator.Loader.InitializeEntitiesAndCollectionsAsync(hydratedObjects[i], reader, session, false, null, parameters[i].UncacheableCollections, cancellationToken)).ConfigureAwait(false);
