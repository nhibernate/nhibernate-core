			...
			for (int i = 0; i < values.Length; i++)
			{
				if (checkability[i])
				{
					...
					if (!nullability[i] && value == null)
					{
						...
						throw new PropertyValueException("not-null property references a null or transient value", persister.MappedClass, persister.PropertyNames[i]);
					}
					else if (value != null)
					{
						...
						{
							throw new PropertyValueException("not-null property references a null or transient value", persister.MappedClass, BuildPropertyPath(persister.PropertyNames[i], breakProperties));
						}
					}
				}
			}
